...
InfluxDBClient client2(INFLUXDB_URL, INFLUXDB_ORG, INFLUXDB_BUCKET, INFLUXDB_TOKEN);
...
void setup_wifi() {
  ...
  Serial.println();
   ...
   if (client2.validateConnection()) {
    Serial.println("Connected to InfluxDB: ");
    Serial.println(client2.getServerUrl());
  } else {
    Serial.println("InfluxDB connection failed: ");
    Serial.println(client2.getLastErrorMessage());
  }
...
void loop() {
  ...
  if (now - lastMeasure > 30000) {
    ...
    sensor.addField("soilMoisture",soilmoisturepercent);
    sensor.addField("waterLevel",sendWater);

    Serial.println(client2.pointToLineProtocol(sensor));
    if (!client2.writePoint(sensor)) {
